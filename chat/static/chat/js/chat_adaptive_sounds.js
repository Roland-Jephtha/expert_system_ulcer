...
// Global variable to track typing sound
...
function startTypingSound() {
    try {
        ...
        typingSound = new Audio('/static/chat/audio/typing.mp3') || 
                     new Audio('../audio/typing.mp3') || 
                     new Audio('audio/typing.mp3');

        // Set up loop for long responses
        typingSound.loop = true;

        // Play the sound
        typingSound.play().catch(e => console.log('Typing sound play error:', e));

        // Set up interval to restart sound if it stops
        typingSoundInterval = setInterval(() => {
            if (typingSound && typingSound.paused) {
                typingSound.play().catch(e => console.log('Typing sound restart error:', e));
            }
...
// Function to estimate typing duration based on text length
function estimateTypingDuration(text) {
    // Base time in ms for short responses
    const baseTime = 2000;

    // Add time based on text length (approximately 20ms per character)
    const textLength = text.length;
    const additionalTime = textLength * 20;

    // Cap at a maximum time (e.g., 30 seconds)
    const maxTime = 30000;
    const totalTime = Math.min(baseTime + additionalTime, maxTime);
        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (questionAnswers[userMsg]) {
                    // Calculate estimated typing duration
                    const estimatedDuration = estimateTypingDuration(questionAnswers[userMsg]);

                    // Hide typing indicator and stop typing sound after estimated duration
                    setTimeout(() => {
                        hideTypingIndicator();
                        stopTypingSound();
                ...
                // Hide typing indicator and stop typing sound
                hideTypingIndicator();
                stopTypingSound();
